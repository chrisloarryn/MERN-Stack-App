...
exports.updatePlaceByIdService = catchAsync(async (req, res, next) => {
  ...
  if (!place || place.length === 0) {
    return next(
      new AppError(
        'Could not find a place to update for the provided place id.',
        404
      )
    )
  }
...
exports.createPlaceService = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        "Creating place failed, please try again(user doesn't exist).",
        500
      )
    )
  }
  ...
  if (!user)
    return next(new AppError('Could not find user for the provided id.', 404))
